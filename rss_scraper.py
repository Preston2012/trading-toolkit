#!/usr/bin/env python3
import requests, hashlib, json, os, time, schedule
...
TRIGGERS = {"ceasefire":("JETS calls / XLE puts","Peace trade"),
...
def scan_rss():
    ...
    if os.path.exists(SEEN_NEWS):
        with open(SEEN_NEWS) as f: seen = json.load(f)
    signals = []
    for url in FEEDS:
        try:
    ...
    with open(SEEN_NEWS,"w") as f: json.dump(seen,f)
    if signals:
