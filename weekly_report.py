#!/usr/bin/env python3
import sqlite3, json, schedule, time, requests, os
...
def weekly_report():
    if not os.path.exists(PAPER_DB): return
    ...
    open_t = conn.execute("SELECT ticker,side,strike,expiry,entry_premium,current_premium,entry_qty FROM paper_trades WHERE status=\'OPEN\'").fetchall()
    closed_t = conn.execute("SELECT ticker,side,strike,expiry,entry_premium,exit_premium,pnl_pct FROM paper_trades WHERE status!=\'OPEN\'").fetchall()
