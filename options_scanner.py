...
def get_technicals(ticker):
    try:
        ...
        support = round(lo20,2)
        resist = round(hi20,2)
        return {"rsi":rsi,"trend":trend,"ema20":ema20,"ema50":ema50,
                "hi20":hi20,"lo20":lo20,"support":support,"resist":resist}
    except: return {}

def composite_score(premium, vol, oi, days, otm_pct, iv, tech, info, side):
    """0-100 score. Higher = better opportunity."""
    s = 0
    # LIQUIDITY (0-25) — can you actually trade this?
    if oi>1000: s+=25
    elif oi>500: s+=22
    elif oi>200: s+=18
    elif oi>50: s+=12
    elif oi>20: s+=6
    # VOLUME (0-20) — is there active interest today?
    if vol>500: s+=20
    elif vol>100: s+=16
    elif vol>50: s+=12
    elif vol>20: s+=8
    # PRICE (0-20) — sweet spot $0.05-$0.30 for max contracts
    if 0.03<=premium<=0.15: s+=20
    elif 0.15<premium<=0.40: s+=16
    elif 0.40<premium<=0.75: s+=12
    elif 0.75<premium<=1.50: s+=8
    # TIME (0-15) — 30-60 days ideal
    if 30<=days<=60: s+=15
    elif 60<days<=90: s+=12
    elif 25<=days<30: s+=8
    elif 90<days<=120: s+=6
    # TECHNICAL ALIGNMENT (0-20) — does the chart agree?
    if tech:
        rsi = tech.get("rsi",50)
        trend = tech.get("trend","")
        if side=="CALL" and trend=="BULL" and rsi<70: s+=20
        elif side=="CALL" and trend=="BULL" and rsi>=70: s+=12  # overbought but trending
        elif side=="PUT" and trend=="BEAR" and rsi>30: s+=20
        elif side=="PUT" and trend=="BEAR" and rsi<=30: s+=12  # oversold but trending
        elif side=="CALL" and trend=="BEAR": s+=5  # contrarian
        elif side=="PUT" and trend=="BULL": s+=5   # contrarian
    # PENALTIES
    if oi<10: s-=15
    if vol<5: s-=10
...
def calc_position(premium, budget=PLAY_BUDGET):
    """Size for $200-500 budget. At $0.10 = 40 contracts on $400."""
    cost_per = premium * 100  # cost per contract
    if cost_per <= 0: return None
    contracts = int(budget / cost_per)
    if contracts < 1: contracts = 1
    total = round(contracts * cost_per, 2)
    # House money ladder: recover basis FIRST, then staged exits
    if contracts >= 20:
        basis_qty = max(1, int(contracts * 0.30))  # 30% sells at 2x to recover ALL cost
...
def paper_enter(ticker, side, strike, expiry, premium, qty):
    conn = init_paper_db()
    # Check if already in this exact trade
    existing = conn.execute("SELECT id FROM paper_trades WHERE ticker=? AND strike=? AND expiry=? AND side=? AND status='OPEN'",
        (ticker, strike, expiry, side)).fetchone()
    if existing: conn.close(); return  # already tracking
    conn.execute("INSERT INTO paper_trades (ticker,side,strike,expiry,entry_premium,entry_date,entry_qty,current_premium,last_updated,status) VALUES (?,?,?,?,?,?,?,?,?,'OPEN')",
        (ticker, side, strike, expiry, premium, datetime.now().isoformat(), qty, premium, datetime.now().isoformat()))
    conn.commit(); conn.close()

def paper_update_prices():
    """Update all open paper trades with current premiums"""
    conn = init_paper_db()
    open_trades = conn.execute("SELECT id,ticker,side,strike,expiry FROM paper_trades WHERE status='OPEN'").fetchall()
    ...
    for tid, ticker, side, strike, expiry in open_trades:
        try:
            ...
            if datetime.now() > exp_dt:
                entry_p = conn.execute("SELECT entry_premium FROM paper_trades WHERE id=?", (tid,)).fetchone()[0]
                conn.execute("UPDATE paper_trades SET status='EXPIRED', exit_premium=0, exit_date=?, pnl_pct=? WHERE id=?",
                    (datetime.now().isoformat(), -100.0, tid))
        except: pass
    conn.commit(); conn.close()

def paper_summary():
    """Weekly P&L summary — sent to TG"""
    conn = init_paper_db()
    trades = conn.execute("SELECT ticker,side,strike,expiry,entry_premium,entry_qty,current_premium,status,pnl_pct FROM paper_trades ORDER BY id DESC LIMIT 20").fetchall()
    conn.close()
    ...
    for t in trades:
        ...
        invested = entry*qty*100; current = cur*qty*100
...
def scan_trump_fresh():
    ...
    try:
        ...
        for n in news[:50]:
            ...
            if h_hash in seen: continue
            hl = headline.lower()
            if not any(kw in hl for kw in ["trump","president","white house"]): continue
...
def run_mega_scan():
    ...
    # Top 5 calls + top 5 puts by score
    ...
    if os.path.exists(LAST_TOP5_FILE):
        with open(LAST_TOP5_FILE) as f: old_keys = json.load(f)
    changed = set(new_keys) != set(old_keys)
    with open(LAST_TOP5_FILE,"w") as f: json.dump(new_keys,f)
    if changed or not old_keys:  # Only send if rankings changed or first run
        if top_calls:
            msg = f"<b>TOP 5 CALLS {ts}</b>\n"
            for r in top_calls: msg += "\n" + fmt(r) + "\n"
            send_tg(msg)
        if top_puts:
            msg = f"<b>TOP 5 PUTS {ts}</b>\n"
            for r in top_puts: msg += "\n" + fmt(r) + "\n"
            send_tg(msg)
    else:
        print("Rankings unchanged — no TG alert")
    # Paper trade all score>=70 automatically
    for r in results:
        if r["score"]>=70 and r.get("pos"):
            paper_enter(r["ticker"],r["side"],r["strike"],r["expiry"],r["premium"],r["pos"]["qty"])
    # Update existing paper trade prices
    paper_update_prices()
    # Trump — FRESH ONLY
...
if __name__ == "__main__":
    ...
    # Trump check every 15 min (fresh only)
    def trump_only():
    ...
    # Paper portfolio update + summary daily at 9pm UTC
    schedule.every().day.at("21:00").do(paper_summary)
    # Paper price updates every hour
